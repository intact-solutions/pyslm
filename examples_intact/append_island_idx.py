...
def build_models():
	...
	'''
		"high_sensi": {"power": 167.62716191366522, "speed": 2.0634244843893508},
		"med_sensi": {"power": 173.18665276441982, "speed": 2.0174683731155607},
		"low_sensi": {"power": 161.93536682904238, "speed": 2.0007891664251285},
		"base": {"power": 152.0, "speed": 2.0785361191810288},
		"boundary": {"power": 161.03299725520932, "speed": 2.017819175094538},
		"interface": {"power": 159.3637716533559, "speed": 2.0880269537424376},
	'''	
	zone_params = {
		"high_sensi": {"power": 160, "speed": 2.},
		"med_sensi": {"power": 160, "speed": 2.},
		"low_sensi": {"power": 160, "speed": 2.},
		"base": {"power": 160.0, "speed": 2.},
...
def main():
	...
	lines = np.loadtxt("youngs_ge_6mm_320_1.txt")
	res = []
	for p in lines:
		...
		if n_z not in island_dict:
			continue
		layer = island_dict[n_z]["layer"]
		bid = island_dict[n_z]["bid"]
		zone_name = get_island_zone_name(layer,1000*p[0],1000*p[1],NEIGHBOR_RADIUS_R,bid)
		if zone_name == "":
			for s in [[1,0],[1,1],[1,-1],[-1,0],[-1,-1],[-1,1],[0,-1],[0,1]]: 
				zone_name = get_island_zone_name(layer,1000*p[0]+s[0],1000*p[1]+s[1],NEIGHBOR_RADIUS_R,bid)
				if zone_name != "":
					break
		new_line = p.tolist()
		new_line.append(zone_name)
		res.append(new_line)
