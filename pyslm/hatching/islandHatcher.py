    ...
    def generateIslands(self, paths, hatchAngle: Optional[float] = 90.0) -> List[Island]:
        ...
        for i in np.arange(0, numIslands):
            for j in np.arange(0, numIslands):
                ...
                _island_width = 0.006
                pos[0][0] = round(pos[0][0]/_island_width)*_island_width # HARD CODE: 6 is the island size
                pos[0][1] = round(pos[0][1]/_island_width)*_island_width # HARD CODE: 6 is the island size
    ...
    def hatch(self, boundaryFeature) -> Layer:
        ...
        if not self._groupIslands:
            ...
            for entry in island_entries:
                if not entry['isIntersecting'] or entry['requiresClipping']:
